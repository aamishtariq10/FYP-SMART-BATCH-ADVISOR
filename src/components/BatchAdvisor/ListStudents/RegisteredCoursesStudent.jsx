...
const RegisteredCoursesStudent = () => {
    const [rows, setRows] = React.useState([]);
    const getRowId = (row) => row.id;

    const params = useParams()
    console.log(params)
    const { StudentRegNo, CurrentSemester } = params
    console.log(params)
    ...
    const getData = async () => {
        try {
            const student = await axios.get(
                `http://localhost:5000/student/request/Coursereg/get/registered/${params.StudentRegNo}/${params.CurrentSemester}`,
                {
                    headers: {
            ...
            >
                <div className="flex flex-col items-start">
                    ...
                    <Typography variant="body1" color="primary">
                        Here you can View Student Registered Courses
                    </Typography>
                </div>
...
export default RegisteredCoursesStudent;
